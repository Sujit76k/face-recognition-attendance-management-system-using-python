...
class Attendance:
    ...
    def __init__(self, root):
        ...
        # Left Frame
        left_frame = LabelFrame(main_frame, bd=2, bg="white", relief=RIDGE, text="Student Information", font=("verdana", 12, "bold"), fg="navyblue")
        ...
        import_btn = Button(btn_frame, command=self.importCsv, text="Import CSV", width=12, font=("verdana", 12, "bold"), fg="white", bg="navyblue")
        import_btn.grid(row=0, column=0, padx=6, pady=10, sticky=W)

        export_btn = Button(btn_frame, command=self.exportCsv, text="Export CSV", width=12, font=("verdana", 12, "bold"), fg="white", bg="navyblue")
        export_btn.grid(row=0, column=1, padx=6, pady=8, sticky=W)

        update_btn = Button(btn_frame, command=self.action, text="Update", width=12, font=("verdana", 12, "bold"), fg="white", bg="navyblue")
        ...
        # ========================== Update Buttons ============================
        # Update button
        update_btn = Button(right_frame, command=self.update_data, text="Update", width=12, font=("verdana", 12, "bold"), fg="white", bg="navyblue")
    ...
    def update_data(self):
        if self.var_id.get() == "" or self.var_roll.get() == "" or self.var_name.get() == "" or self.var_time.get() == "" or self.var_date.get() == "" or self.var_attend.get() == "Status":
            ...
            try:
                Update = messagebox.askyesno("Update", "Do you want to update the student's attendance?", parent=self.root)
                if Update > 0:
    ...
    def action(self):
        if self.var_id.get() == "" or self.var_roll.get() == "" or self.var_name.get() == "" or self.var_time.get() == "" or self.var_date.get() == "" or self.var_attend.get() == "Status":
            ...
            try:
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo("Success", "All records have been saved to the database!", parent=self.root)
            except Exception as es:
                messagebox.showerror("Error", f"Due to: {str(es)}", parent=self.root)
