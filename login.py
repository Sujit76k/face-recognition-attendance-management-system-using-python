...
class Login:
    def __init__(self, root):
        ...
        # self.bg = ImageTk.PhotoImage(file=r"D:\Downloads\Attendance_Management_System_Using_Face_Recognition-main\Attendance_Management_System_Using_Face_Recognition-main\Images_GUI\loginBg1.jpg")
        ...
        try:            
            self.bg = ImageTk.PhotoImage(file=r"D:\Downloads\Attendance_Management_System_Using_Face_Recognition-main\Attendance_Management_System_Using_Face_Recognition-main\Images_GUI\loginBg.jpg")
        ...
        img1 = Image.open(r"D:\Downloads\Attendance_Management_System_Using_Face_Recognition-main\Attendance_Management_System_Using_Face_Recognition-main\Images_GUI\login1.png")
        img1 = img1.resize((100, 100), Image.Resampling.LANCZOS)
        self.photoimage1 = ImageTk.PhotoImage(img1)
        lb1img1 = Label(image=self.photoimage1, bg="#002B53")
        lb1img1.place(x=610, y=125, width=100, height=100)
        ...
        # Register Button
        regbtn = Button(frame1, command=self.reg, text="Register", font=("times new roman", 10, "bold"), bd=0, relief=RIDGE, fg="white", bg="#002B53", activeforeground="orange", activebackground="#002B53")
        regbtn.place(x=33, y=370, width=50, height=20)

        # Forgot Password Button
        forgetbtn = Button(frame1, command=self.forget_pwd, text="Forgot Password", font=("times new roman", 10, "bold"), bd=0, relief=RIDGE, fg="white", bg="#002B53", activeforeground="orange", activebackground="#002B53")
        forgetbtn.place(x=90, y=370, width=100, height=20)
    ...
    def login(self):
        ...
        elif self.txtuser.get() == "sk@gmail.com" and self.txtpwd.get() == "admin":
            try:
                self.new_window = Toplevel(self.root)
    ...
    def reset_pass(self):
        if self.var_ssq.get() == "Select":
            messagebox.showerror("Error", "Please choose a security question!", parent=self.root2)
            ...
            conn = mysql.connector.connect(username='root', password='sujit', host='localhost', database='face_recognition', port=3306)
            mycursor = conn.cursor()
            query = ("SELECT * FROM regteach WHERE email=%s AND security_question=%s AND answer=%s")
            ...
            if row is None:
                ...
                conn.commit()
                conn.close()
                messagebox.showinfo("Info", "Your password has been successfully reset. Please login with the new password!", parent=self.root2)

    # Forgot Password Window
    def forget_pwd(self):
        if self.txtuser.get() == "":
            messagebox.showerror("Error", "Please enter your email ID to reset password!")
        else:
            conn = mysql.connector.connect(username='root', password='sujit', host='localhost', database='face_recognition', port=3306)
            mycursor = conn.cursor()
...
class Face_Recognition_System:
    def __init__(self, root):
        ...
        hlp_b1_1 = Button(bg_img, command=self.helpSupport, text="Help", cursor="hand2", font=("tahoma", 15, "bold"), bg="white", fg="navyblue")
        ...
        try:
            ...
            print("Error: tra1.jpg not found.")
        except Exception as e:
            print(f"Error loading tra1.jpg: {e}")
        ...
        # pho_img_btn = Image.open(r"D:\Downloads\Attendance_Management_System_Using_Face_Recognition-main\Attendance_Management_System_Using_Face_Recognition-main\qr1.png")
        ...
        #     self.pho_img1 = ImageTk.PhotoImage(pho_img_btn)
        # except FileNotFoundError:
        #     print("Error: qr1.png not found.")
        #     # Optionally, display a placeholder image or disable the button.
        # except Exception as e:
        #     print(f"Error loading qr1.png: {e}")
        ...
        # Photos Button
        self.pho_img1 = None  # Initialize to None
        ...
        try:
            ...
            # placeholder_img = Image.new("RGB", (180, 180), "gray") # Create a gray image
            # self.pho_img1 = ImageTk.PhotoImage(placeholder_img)
        except Exception as e:
            print(f"Error loading qr1.png: {e}")
            # Optionally, display an error message.

        pho_b1 = Button(bg_img, command=self.open_img, 
                        image=self.pho_img1 if self.pho_img1 else "",  # Use pho_img1 if available, otherwise an empty string
                        cursor="hand2")
        pho_b1.place(x=450, y=300, width=180, height=180)
